...
- [ ] Аудит всех модалок: поля не должны наезжать, локально задать ширины где нужно.

---

## 3) Управление характеристиками

- [ ] Для формы “Новое поле” применить `.char-field-grid`, двухколоночные RU/EN, тип поля на новой строке.
- [ ] Всегда обновлять select шаблонов после load/save/delete/import.
- [ ] Действия таблицы полей — иконками (ширина 120–140px).
...
- [ ] Убедиться что строка 44px и действия иконками везде.

---

## 8) Фильтры по кастомным полям (проекты/группы)
- [ ] Кнопка “Фильтры” (funnel) рядом с поиском, открывает drawer.
- [ ] Видимость поля если привязано >1 сущности; фильтры по типу (text/number/select/bool) с логикой из ТЗ.
...
- [ ] Модалки: поля не налезают.
- [x] Этап collapsed ≤2 строк, header + чипы.
- [x] Задачи 1 строка, meta строка, подзадачи раскрываются.
- [ ] Compact tasks table ровная, sticky/width соблюдены.
- [ ] Архитектура не сломана, id/классы сохранены.
