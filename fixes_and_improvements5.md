...
- [x] Убедиться, что фильтры по пользовательским полям работают в связке (AND) и покрыты автотестами для проектов и групп; исключить падения при пустом значении дополнительного ключа.

### 8.3. Ширина попапов

- [x] Вернуть ширину `width: 100%` всем диалогам, в том числе формам создания/редактирования проектов и групп, чтобы верстка не сужалась.

### 8.4. Отображение групп

- [x] Список групп отображается сразу после загрузки страницы и не скрывается при сбоях загрузки метаданных кастомных полей; ошибки не блокируют обновление списка.

---

## 9. Дополнительные корректировки (фильтры и формы)

- [x] Выровнять попапы расширенных фильтров проектов и групп по центру экрана и ограничить ширину примерно 650px без растягивания.
- [x] Сохранить `width: 100%` у общих диалогов, но убрать его из инпутов/селектов форм создания/редактирования проектов и групп, чтобы поля не занимали всю ширину по умолчанию.
- [x] Установить для топбара единый `z-index: 5`, чтобы он не перекрывал попапы и фильтры.
- [x] Обеспечить отображение групп даже при временных ошибках загрузки данных, используя локальный кеш последнего успешного ответа.

---

## 10. Дополнительные правки после отчётов

### 10.1. Характеристики — липкость меню и действия

- [x] Снять липкость верхнего меню на странице характеристик, чтобы оно не фиксировалось при скролле.
- [x] Переключить действия «Сохранить» и «Удалить» в секциях на иконки без текстовых подписей.

### 10.2. Характеристики — форма нового поля

- [x] Выровнять поля Label RU, Label EN и «Тип поля» в одну строку без наложений и без принудительного `width: 100%`.

## 11. Шаблоны — детальная карточка

- [x] Убрать принудительную ширину `width: 100%` у полей внутри `.template-detail`, сохранив выравнивание и общий стиль элементов.
- [x] Увеличить поле описания и добавить зазор между подписью и вводом, чтобы текст не прилегал к лейблу.

## 12. Характеристики — компактные действия в таблице

- [x] Сделать кнопки действий в таблице полей иконками без обводок, сократив их габариты.
- [x] Уменьшить ширину колонки действий, сохранив выравнивание и читаемость.

## 13. Компактный список задач

- [x] Добавить явную кнопку удаления в таблице компактного списка задач.
- [x] Показать иконки действий (редактирование, комментарии, статус, удаление) в компактной таблице и карточках, чтобы удаление было
  всегда доступно.

---

## 14. Актуализация данных и порядок пользовательских полей

- [x] Исключить устаревший кэш при загрузке задач и изображений проектов, пробрасывая no-store и кеш-ключи для обложек/миниатюр.
- [x] Добавить перестановку пользовательских полей в карточке проекта (вверх/вниз) с сохранением порядка отображения.

## 15. Производительность страницы проекта

- [x] Включить ленивую загрузку и асинхронное декодирование эскизов галереи, чтобы скролл не подвисал на длинных списках изображений.
- [x] Добавить content-visibility для карточек галереи, сохранив их визуальный стиль и снижая нагрузку при прокрутке больших коллекций.
- [x] Перерисовывать галерею пакетами через requestIdleCallback и отложенную подгрузку превью по IntersectionObserver, чтобы не блокировать скролл даже при сотнях снимков.

## 16. Большие изображения галереи

- [x] Сжимать и масштабировать загружаемые фото до разумного размера перед отправкой, чтобы 5–15 МБ исходники не тормозили интерфейс при просмотре.
- [x] Сохранять PNG-изображения в исходном формате при сжатии/масштабировании, не конвертируя их в JPEG.
