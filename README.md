...
## Быстрые проверки регрессии и логи

- Для фиксации потенциальных 500-ошибок бэкенд пишет ротационные логи в `data/logs/app.log`.
- Быстрый прогон ключевых сценариев доступен через `python backend/smoke_tests.py` — скрипт поднимает временное хранилище,
  проходит по API (группы, проекты, GTM, характеристики, задачи, комментарии, бэкапы) и проверяет, что в логах нет ошибок.
