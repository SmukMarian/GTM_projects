...
- Переключатель тёмной/светлой темы с цветами в духе ChatGPT расположен в правом верхнем углу каждой страницы.
- Выбор темы сохраняется в `localStorage` и учитывает системный `prefers-color-scheme` при первом открытии.
- Если пользователь не менял тему вручную, страница автоматически подстраивается под смену системных настроек и нормализует сохранённое значение при загрузке.
